...
import matroid.constructions matroid.projection ftype.minmax ftype.setlist
...
/-- type of disjoint pairs of sets in U -/
...
def is_union_two_indep (M₁ M₂ : matroid U) : set U → Prop := 
...
@[simp] lemma sum_ranks_sets_empty_list (X : list (set U)): 
  sum_of_ranks_of_sets (list.nil) X = 0 := 
by {unfold sum_of_ranks_of_sets, simp, }

@[simp] lemma sum_ranks_set_empty_list (X : set U): 
  sum_of_ranks_of_set (list.nil) X = 0 := 
by {unfold sum_of_ranks_of_set, simp, }


instance indep_list_nonempty (Ms : list (matroid U)) : nonempty (indep_list Ms) := 
begin
...
instance indep_list_fintype (Ms : list (matroid U)) : fintype (indep_list Ms) := 
begin
  set f : indep_list Ms → vector (set U) (Ms.length):= λ Xs, ⟨Xs.val, Xs.property.1⟩ with hf, 
...
lemma indep_list_nil_elim {Xs : list (set U)}:
  is_indep_list (list.nil : list (matroid U)) Xs → Xs = list.nil := 
by {rintros ⟨hx,hx'⟩, simp [list.length_eq_zero] at hx, from hx,} 

lemma indep_list_nil_iff_nil {Xs : list (set U)}:
  is_indep_list (list.nil : list (matroid U)) Xs ↔ Xs = list.nil := 
by {refine ⟨λ h, indep_list_nil_elim h, λ h,_⟩, unfold is_indep_list, rw h, simp, } 


lemma indep_list_nil_elim_subtype (Xs : indep_list (list.nil : list (matroid U))):
  Xs.val = list.nil := 
...
lemma union_of_indep_cons (Ms : list (matroid U))(N : matroid U)(X : set U):
