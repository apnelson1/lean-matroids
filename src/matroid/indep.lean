...
lemma fincard_ind_le_fincard_set_basis {M : indep_family α} {I B X : set α} :
  I ⊆ X → M.indep I → M.is_set_basis B X → fincard I ≤ fincard B := 
  begin
...
lemma set_bases_equicardinal {M : indep_family α} {X B₁ B₂ : set α} :
  M.is_set_basis B₁ X → M.is_set_basis B₂ X → fincard B₁ = fincard B₂ :=
begin
  intros h₁ h₂, apply le_antisymm, 
  from fincard_ind_le_fincard_set_basis h₁.1 h₁.2.1 h₂, 
  from fincard_ind_le_fincard_set_basis h₂.1 h₂.2.1 h₁, 
end 
...
begin
  ...
  have := fincard_ind_le_fincard_set_basis hI hInd this, 
  have : M.I_to_r X = fincard J := rfl, 
  linarith, 
end 

lemma I_to_r_eq_iff {M : indep_family α} {X : set α} {n : ℤ} :
  M.I_to_r X = n ↔ ∃ B, M.is_set_basis B X ∧ fincard B = n :=
  let B₀ := M.choose_set_basis X, hB₀ := M.choice_of_set_basis_is_valid X in 
begin
  refine ⟨λ h, ⟨B₀, ⟨hB₀,by {rw ←h, refl}⟩⟩, λ h,_⟩, 
  rcases h with ⟨B, ⟨hB, hBfincard⟩⟩, 
  from (rfl.congr hBfincard).mp (eq.symm (set_bases_equicardinal hB hB₀)), 
end

lemma has_set_basis_with_fincard (M : indep_family α) (X : set α) : 
  ∃ B, M.is_set_basis B X ∧ fincard B = M.I_to_r X :=
  I_to_r_eq_iff.mp (rfl : M.I_to_r X = M.I_to_r X)
 
lemma I_to_r_of_set_basis {M : indep_family α} {B X : set α} :
  M.is_set_basis B X → M.I_to_r X = fincard B := 
...
λ X, by {have := I_to_r_ub (empty_subset X) (M.I1), rw fincard_empty at this, assumption}

lemma I_to_R1 (M : indep_family α) : 
  satisfies_R1 M.I_to_r := 
λ X, by {rcases M.I_to_r_max X with ⟨B, ⟨hBX, ⟨_, hsB⟩⟩⟩, from (eq.symm hsB).trans_le (fincard_monotone hBX)}

lemma I_to_r_of_indep (M : indep_family α) (I : set α) : 
  M.indep I → M.I_to_r I = fincard I :=
...
begin
  ...
  { have := I_to_r_ub (inter_subset_right Bα X) (M.I2 _ _ (inter_subset_left Bα X) hαb.2.1),
    ...
    exact eq_of_le_fincard_subset (subset_inter hαs h.1) this, },
...
begin
  ...
  have := fincard_monotone hBα.1, have := fincard_monotone (subset_inter hss hBIBY),
  linarith[fincard_modular BX BY]
end 
...
begin
  ...
  rcases MI.has_set_basis_with_fincard X with ⟨B, ⟨hB₁, hB₂, hB₃⟩, hBX⟩, 
  refine ⟨λ h, _, λ h, _⟩,
  rw [←eq_of_eq_fincard_subset hB₁ _], from hB₂, 
  from eq.trans hBX h, 
