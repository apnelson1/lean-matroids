...
lemma R0 {U : boolalg}(M : rankfun U)(X : U) : 
  0 ≤ M.r X :=
  M.R0 X 

lemma R1 {U : boolalg}(M : rankfun U)(X : U) : 
  M.r X ≤ size X :=
  M.R1 X 

lemma R2 {U : boolalg}(M : rankfun U){X Y : U} : 
  X ⊆ Y → M.r X ≤ M.r Y :=
  M.R2 X Y 

lemma R3 {U : boolalg}(M : rankfun U)(X Y : U) : 
...
lemma C2 (M : rankfun U){C₁ C₂ : U}:
  ...
  λ hC₁ hC₂ hC₁C₂, hC₁C₂.2 (C2' M hC₁C₂.1 hC₁ hC₂)
...
lemma C3 {M : rankfun U} {C₁ C₂ : U} {e : single U}: 
  ...
  begin
    intros hC₁C₂ hC₁ hC₂ he, rw [←dep_iff_contains_circuit, dep_iff_r], 
...
begin
  ...
  from ⟨λ h, ⟨h.2.1,λ e, h.2.2 e (subset_top e)⟩, λ h, ⟨subset_top B, ⟨h.1,λ e he,h.2 e⟩⟩ ⟩, 
end

lemma basis_of_iff_augment_i {M : rankfun U}{B X : U} : 
  is_basis_of M B X ↔ B ⊆ X ∧ is_indep M B ∧ ∀ e, e ∈ X \ B → ¬is_indep M (B ∪ e) :=
begin
