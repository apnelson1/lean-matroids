...
/-- the fincard of a projective geometry over a `q`-element field; i.e `1 + q + q^2 + ... + q^{n-1}` -/
def pg_fincard' : β → ℕ → β
| q 0     := 0
| q (n+1) := 1 + q * (pg_fincard' q n)

@[simp] lemma pg_fincard'_zero (q : β) : pg_fincard' q 0 = 0 := rfl 

@[simp] lemma pg_fincard'_succ {q : β}{n : ℕ}: pg_fincard' q n.succ = 1 + q * (pg_fincard' q n) := rfl 

@[simp] lemma pg_fincard'_order_one (n : ℕ) : pg_fincard' (1 : ℤ) n = n := 
by {induction n with n ih, simp, simp [pg_fincard', ih, add_comm]  }

lemma pg_fincard'_eq_powsum (q : β) (n : ℕ) : pg_fincard' q n = ∑ᶠ i in Ico 0 n , q^i  := 
begin
  induction n with n ih, 
    simp, 
  rw [pg_fincard', ih], rw mul_distrib_finsum_in q, 
...
lemma pg_fincard'_rat (q : ℤ) (n : ℕ) (hq : 2 ≤ q) : (pg_fincard' q n : ℚ) = (q^n - 1)/(q - 1) := 
begin
  induction n with n ih, 
    simp, 
  replace hq : (q : ℚ) -1 ≠ 0 := by {norm_cast, linarith}, 
  rw [pg_fincard', ih, ← mul_div_assoc, one_add_div hq, div_left_inj' hq, pow_succ], 
  ring, 
end

/- the fincard of a rank-`n` projective geometry over `GF(q)`, or equivalently the value of the sum
`1 + q + q² + q³ + ... + q^{n-1}` For convenience, this is defined for all integers `n`, taking 
the value `0` if `n ≤ 0`.  -/
def pg_fincard (q n : ℤ) := pg_fincard' q n.to_nat 

lemma pg_fincard_eq_zero (q n : ℤ)(hn : n ≤ 0) : pg_fincard q n = 0 := 
by {rw pg_fincard, convert pg_fincard'_zero q, rw int.to_nat_zero_of_nonpos hn, }

lemma pg_fincard_rec (q : ℤ) {n : ℤ} (hn : 1 ≤ n) : pg_fincard q n = 1 + q * (pg_fincard q (n-1)) :=
begin
  rw [pg_fincard, pg_fincard], 
  obtain ⟨m,rfl⟩ := int.eq_coe_of_zero_le (by linarith : 0 ≤ n), 
  norm_num, 
  cases m, exfalso, norm_num at hn, refl, 
end

lemma pg_fincard_eq_powsum (q n : ℤ) : pg_fincard q n = ∑ᶠ i in Ico 0 n.to_nat, q^i  := 
begin
  cases le_or_lt n 0 with hn hn', 
  { convert finsum_in_empty.symm, 
    exact pg_fincard_eq_zero q n hn, 
    apply Ico_eq_empty, 
    rw int.to_nat_zero_of_nonpos hn,  },
  convert pg_fincard'_eq_powsum q n.to_nat, 
end

lemma pg_fincard_nonneg (q n : ℤ)(hq : 0 ≤ q): 0 ≤ pg_fincard q n :=
by {rw pg_fincard_eq_powsum, exact nonneg_of_finsum_in_nonneg (λ i hi, pow_nonneg hq i)}
...
begin
  ...
  { convert fin.finsum_in_le_finsum_in h_L_ub, 
    rw [int.finsum_in_const_eq_mul_fincard, hlines, ε_proj_nonloop _ he]}, 
  
  /- Since `M ⟋ e` isn't a counterexample, it doesn't have too many points  -/
  specialize hM_min (M ⟋ {e}) 
    (fincard_strict_monotone (project_nonloop_fewer_nonloops he))
  ...
  have hlt := calc 
    pg_fincard q (M.r univ)  < 1 + ∑ᶠ (L : set α) in lines, (M.ε L - 1) 
                        : hMs 
                      ... ≤ 1 + q * (M ⟋ {e}).ε univ                 
                        : add_le_add_left h_le 1
                      ... ≤ 1 + q * (pg_fincard q ((M ⟋ {e}).r univ))   
                        : add_le_add_left ((mul_le_mul_left (by linarith : 0 < q)).mpr hM_min) 1,

  rw [project_r, univ_union, rank_nonloop he, ← pg_fincard_rec q (by linarith : 1 ≤ M.r univ)] at hlt, 
...
/-
  ...
  { by_contra hn', push_neg at hn', obtain ⟨e,f,hef,hne⟩ := hn', 
    set M' := M ⟍ {f} with hM', 
    specialize hM_min M' 
      (fincard_strict_monotone (loopify_nonloop_fewer_nonloops hef.nonloop_right))
