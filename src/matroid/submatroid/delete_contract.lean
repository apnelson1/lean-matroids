...
def restrict (M : matroid_in α) (R : set α) : matroid_in α := M.delete Rᶜ 

notation M / C := matroid_in.contract M C 
notation M \ D := matroid_in.delete M D 
notation M ∣ R := matroid_in.restrict M R 

lemma restr_eq_del_compl (M : matroid_in α) (R : set α) :
  M ∣ R = M \ Rᶜ := 
rfl 

@[simp, msimp] lemma con_rank (M : matroid_in α) (C X: set α) : 
  (M / C).r X = M.r (X ∪ C) - M.r C :=
M.carrier.project_r _ _ 

@[simp, msimp] lemma con_E (M : matroid_in α) (C: set α) :
  (M / C).E = M.E \ C := 
rfl 

@[simp, msimp] lemma del_rank (M : matroid_in α) (D X: set α) : 
  (M \ D).r X = M.r (X \ D) :=
rfl

@[simp, msimp] lemma restr_r (M : matroid_in α) (R X : set α) :
  (M ∣ R).r X = M.r (X ∩ R) := 
by rw [restr_eq_del_compl, del_rank, diff_compl]

@[simp, msimp] lemma del_E (M : matroid_in α) (D: set α) :
  (M \ D).E = M.E \ D := 
rfl 

@[simp, msimp] lemma restr_E (M : matroid_in α) (R : set α) : 
  (M ∣ R).E = M.E ∩ R := 
by rw [restr_eq_del_compl, del_E, diff_compl]

lemma contr_restr_E (M : matroid_in α){C R : set α} (hR : R ⊆ M.E \ C) : 
  (M / C ∣ R).E = R := 
by rwa [restr_E, con_E, ←subset_iff_inter_eq_right]

@[simp, msimp] lemma con_empty (M : matroid_in α) : 
  (M / ∅) = M := 
by {ext, simp, intros X hX, simp [←r_carrier_eq_r _ ∅]}

@[simp, msimp] lemma del_empty (M : matroid_in α) : 
  (M \ ∅) = M := 
by {ext, simp, intros X hX, simp [←r_carrier_eq_r _ ∅]}

@[simp,msimp] lemma del_del (M : matroid_in α) (D D' : set α) :
  (M \ D \ D') = M \ (D ∪ D') := 
by {ext, simp [diff_eq, ←inter_assoc], intros X hX, simp [diff_eq, ←inter_assoc, inter_right_comm ],  }

lemma del_del_comm (M : matroid_in α) (D D' : set α) : 
  M \ D \ D' = M \ D' \ D := 
by rw [del_del, union_comm, ←del_del]

lemma del_eq_del_inter_E (M : matroid_in α) (D : set α) : 
  M \ D = M \ (M.E ∩ D) :=
begin
  ext, 
...
lemma con_eq_con_inter_E (M : matroid_in α) (C : set α) : 
  M / C = M / (M.E ∩ C) := 
begin
  ext,
  {  simp only [diff_eq] with msimp, rw [compl_inter, inter_distrib_left], simp,  },
...
lemma con_del_E (M : matroid_in α) (C D : set α) : 
  (M / C \ D).E = M.E \ (C ∪ D) :=
by {simp only [diff_eq] with msimp, simp [←inter_assoc]}

lemma con_del_eq_del_con (M : matroid_in α) (C D : set α) (h : C ∩ D = ∅) : 
  M / C \ D = M \ D / C := 
begin
...
lemma con_del_eq_del_con' (M : matroid_in α) (C D : set α) :
  M / C \ D = M \ (D \ C) / C :=
begin
...
lemma del_con_eq_con_del' (M : matroid_in α) (C D : set α) :
  M \ D / C = M / (C \ D) \ D :=
begin
...
lemma dual_con_eq_del_dual (M : matroid_in α) (A : set α) (hA : A ⊆ M.E) : 
...
lemma dual_con (M : matroid_in α) (A : set α) : 
  (M / A).dual = (M.dual) \ A :=
begin
...
lemma dual_del (M : matroid_in α) (A : set α) :
  (M \ A).dual = M.dual / A :=
by rw [←M.dual_dual, ←dual_con, matroid_in.dual_dual, matroid_in.dual_dual]

lemma con_con (M : matroid_in α) (C C' : set α) :
  (M / C / C') = M / (C ∪ C') :=
by {rw [←M.dual_dual], repeat {rw ←dual_del}, rw del_del}

lemma con_con_comm (M : matroid_in α) (C C' : set α) : 
  M / C / C' = M / C' / C:= 
by rw [con_con, union_comm, ← con_con]

lemma con_del_con_del (M : matroid_in α) (C C' D D': set α) (h : (C ∪ C') ∩ (D ∪ D') = ∅ ) :
  M / C \ D / C' \ D' = M / (C ∪ C') \ (D ∪ D') :=
begin
...
lemma con_rank_ground (M : matroid_in α) (C : set α) :
...
lemma indep_del_iff {M : matroid_in α} {D X : set α} : 
...
lemma delete_coindep_rank_ground (M : matroid_in α){D : set α} (hD : M.dual.is_indep D) :
  (M \ D).r (M \ D).E = M.r M.E := 
by {simp only [inter_assoc, inter_self, diff_eq] with msimp, exact (coindep_iff_r.mp hD).2,} 

lemma coindep_contract_iff {M : matroid_in α} {C X : set α} : 
...
@[ext] structure cd_pair (M : matroid_in α) :=
(C : set α)
(D : set α)
(disj : C ∩ D = ∅)
(C_ss_E : C ⊆ M.E)
...
def cd_pair.switch {M : matroid_in α} (p : cd_pair M ) : cd_pair M  :=
...
@[simp, msimp] lemma cd_pair.rank_subtype {M : matroid_in α} (p : cd_pair M) (X : set (M / p.C \ p.D).E) : 
  (M / p.C \ p.D).r X = M.r (X ∪ p.C) - M.r p.C := 
by {rw cd_pair.rank, rintro x hx, obtain ⟨⟨x,h⟩,-,rfl⟩ := (mem_image _ _ _).mp hx, exact h, }

@[simp, msimp] lemma cd_pair.E {M : matroid_in α} (p : cd_pair M) : 
  (M / p.C \ p.D).E = M.E \ (p.C ∪ p.D) := 
by simp only [diff_diff] with msimp

def cd_pair.of_restr (M : matroid_in α){R : set α} (hR : R ⊆ M.E) : 
  cd_pair M := 
{ C := ∅, 
  D := M.E \ R, 
...
def cd_pair.of_con_restr (M : matroid_in α){C R : set α} (hC : C ⊆ M.E) (hR : R ⊆ M.E \ C) : 
  cd_pair M :=
{ C := C, 
  D := M.E \ (C ∪ R), 
  C_ss_E := hC, 
...
def cd_pair.from_as_mat {M : matroid_in α} (p : cd_pair (M.as_mat : matroid_in M.E)) : cd_pair M := 
{ C := coe '' p.C,
  D := coe '' p.D,
...
def cd_pair.image {M : matroid α} {M' : matroid β} (p : cd_pair (M : matroid_in α)) (f : α ↪ β) : 
