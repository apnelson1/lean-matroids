...
The advantage of this over taking minors is that we stay in the Type `matroid α` rather than 
changing the ground set and having to deal with the type theory that results. In practice it seems 
...
section project 
variables {α : Type*} [fintype α] {e f : α} {M : matroid α} {X Y D C F I R: set α}
...
{ r := λ X, M.r (X ∪ C) - M.r C,
  ...
  R2 := λ X Y hXY, by {simp only, linarith [M.rank_mono (union_subset_union_left C hXY)]},
