...
begin
  simp_rw indep_iff_r, 
  refine ⟨λ h, λ X hX, le_antisymm (M.rank_le_fincard _) _, λ h, λ Y, _⟩,  
  {  rw ←hX, apply h}, 
...
begin
  ...
  apply nonneg_int_strong_induction_param P (λ p, fincard (p.2 \ p.1)), 
  { rintros ⟨X,Y⟩, apply fincard_nonneg}, 
  { rintros ⟨X,Y⟩ hs hXY, dsimp only at *, 
    rw [fincard_zero_iff_empty, diff_empty_iff_subset] at hs, 
    rw [subset.antisymm hXY hs, sub_self, sub_self],  }, 
  rintros ⟨X,Y⟩ h_fincard h' hXY, dsimp only at *,   
  cases exists_mem_of_fincard_pos h_fincard with e he,
  specialize h' ⟨X, Y \ {e}⟩ _ _, 
  { dsimp only, rw [diff_right_comm, fincard_remove_mem he], linarith}, 
