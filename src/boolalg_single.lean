...
def single : Type := {X : α // size X = 1}

instance coe_single : has_coe single α := ⟨λ e, e.val⟩  

def elem : @single α _ → α → Prop := λ e X, (e : α ) ⊆ X 
def nonelem : @single α _ → α → Prop := λ e X, ¬elem e X 

infix ∈ := elem 
infix ∉ := nonelem 

@[simp] lemma nonelem_simp {e : single}{X : α}: 
  ¬(e ∈ X) ↔ e ∉ X := by refl  

lemma elem_iff {e : single}{X : α} :
   e ∈ X ↔ (e:α) ⊆ X := 
  by unfold elem

lemma nonelem_iff {e : single}{X : α} : 
  e ∉ X ↔ ¬(e:α) ⊆ X := 
  by refl 

@[simp] lemma size_coe_single (e : @single α _) :
   size (e : α) = 1 := e.2 

@[simp] lemma single_ne_bot (e : @single α _) : 
  (e:α) ≠ ⊥ := 
  λ h, by {have := size_coe_single e, rw [h,size_bot] at this, linarith}

@[simp] lemma nonelem_bot (e : single) : 
  e ∉ (⊥:α) := 
  by {rw nonelem_iff, intro h, replace h := size_monotone h, simp at h, linarith}

@[simp] lemma nonelem_compl (e : single) :
  e ∉ (e:α)ᶜ := 
  λ h, single_ne_bot e (subset_own_compl h)

lemma nonempty_has_elem {X : α}  : 
  X ≠ ⊥ → ∃ e, e ∈ X := 
  λ hX, by {rcases single_subset_nonempty hX with ⟨Y,Z ,⟨hI,hU,h1⟩⟩, use ⟨Y,h1⟩, 
                rw ←hU, exact subset_union_left Y Z}

lemma nonempty_iff_has_elem {X : α} : 
  X ≠ ⊥ ↔ ∃ e, e ∈ X :=
  ⟨λ h, nonempty_has_elem h, λ h, λ hb, by {cases h with e he, rw hb at he, exact nonelem_bot e he}⟩ 

lemma size_pos_has_elem {X : α}: 
  0 < size X → ∃ e, e ∈ X := 
  λ h, (nonempty_iff_has_elem.mp (λ h', by {rw [h',size_bot] at h, exact lt_irrefl 0 h}))

lemma size_pos_iff_has_elem {X : α}: 
  0 < size X ↔ ∃ e, e ∈ X := 
  ⟨λ h, size_pos_has_elem h, λ h, by {cases h with e he, have := size_monotone he, rw size_coe_singlet this, linarith}⟩ 

lemma size_zero_iff_has_no_elem {X : α}:
  size X = 0 ↔ ¬ ∃ e, e ∈ X := 
  begin
...
lemma nested_singles_eq {e f: single} :
  (e: α) ⊆ (f :α) → e = f :=
  begin
...
lemma nonelem_compl_iff {X : α}{e : single} :
   e ∉ Xᶜ ↔ e ∈ X  := 
  by {rw ←elem_compl_iff, rw [compl_compl]}

lemma elem_union_iff {e : single} {X Y : α} : 
  e ∈ X ∪ Y ↔ e ∈ X ∨ e ∈ Y :=
   begin 
...
lemma subset_iff_elems_contained {X Y : α} :
  X ⊆ Y ↔ ∀ e, e ∈ X → e ∈ Y := 
  begin
...
lemma removal_subset_of {X Y : α}{e : single} :
  ...
  begin
    ...
    nth_rewrite 1 ← h,
    rw [inter_distrib_left, inter_distrib_right, inter_assoc _ (e:α), inter_right_comm _ _ Y], 
    simp only [inter_compl, union_bot, inter_bot], 
  end

lemma ssub_of_add_compl {X : α} {e : single} : 
  e ∈ Xᶜ → X ⊂ X ∪ e := 
  begin
...
lemma ssub_of_add_nonelem {X : α} {e : single}: 
  e ∉ X → X ⊂ X ∪ e := 
  λ hXe, ssub_of_add_compl (elem_compl_iff.mpr hXe)

lemma ssubset_of_add_nonelem_iff {X : α} {e : single} :
  e ∉ X ↔ X ⊂ X ∪ e :=
...
lemma remove_single_size {X :α}{e : single} :
  e ∈ X → size (X \ e) = size X - 1 := 
begin
  intro heX, 
...
lemma add_from_nonempty_diff {X Y : α} :
  X ⊂ Y ↔ ∃e, e ∉ X ∧ X ∪ e ⊆ Y := 
  begin
    refine ⟨λ h,_, λ h, _⟩, 
...
lemma aug_of_ssubset {X Y : α} : 
  X ⊂ Y → ∃ Z (e : single), X ⊆ Z ∧ Z ⊂ Y ∧ Z ∪ e = Y :=
  begin
    intro hXY, 
...
lemma exchange_comm {X : α}{e f : single} : 
  e ∈ X → f ∉ X → (X \ e) ∪ f = (X ∪ f) \ e := 
  begin
    intros he hf, 
...
lemma exchange_size {X : α}{e f : single} : 
  e ∈ X → f ∉ X → size ((X \ e) ∪ f) = size X := 
  λ he hf, by linarith [add_nonelem_size (nonelem_diff_of_nonelem (e:α) hf),remove_single_size he]

lemma exchange_pair_sizes {X Y : α}{e f : single}: 
...
lemma size_union_distinct_singles {e f : single}: 
  e ≠ f → size (e ∪ f :α) = 2 :=
  begin
    intros hef, 
    have : ¬((e:α) ⊆ (f:α)) := λ h, hef (nested_singles_eq h), 
    have := add_nonelem_size this, 
    rw [union_comm, size_coe_single] at this, 
    linarith, 
  end 

lemma size_union_singles_lb (e f : single): 
  1 ≤ size (e ∪ f : α) := 
  by {linarith [size_monotone (subset_union_left (e:α) f), size_coe_single e]}

lemma size_union_singles_ub (e f : single):
  size (e ∪ f :α) ≤ 2 := 
  by {by_cases e = f, rw [h, union_idem, size_coe_single], linarith, linarith [size_union_distinct_singles h]}

lemma subset_single {e : single}{X : α} :
  X ⊆ e → X = ⊥ ∨ X = e := 
  begin
...
lemma ssubset_pair {e f : single}{X : α}:
  X ⊂ (e ∪ f : α) → X = ⊥ ∨ (X = e) ∨ (X = f) :=
  begin
...
lemma equal_or_single_in_diff {X Y : α} :
