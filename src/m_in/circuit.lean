...
begin
  ...
  exact mem_cl_of_mem' _ (mem_singleton _), 
end 
...
lemma circuit.strong_multi_elimination {ι : Type*} (hC : M.circuit C) (x : ι → E) (Cs : ι → set α) 
...
lemma eq_of_circuit_iff_circuit_forall {M₁ M₂ : matroid_in α} (h : ∀ C, M₁.circuit C ↔ M₂.circuit C) :
  M₁ = M₂ :=
