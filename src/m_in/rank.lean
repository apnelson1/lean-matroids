...
/-- The rank `r X` of a set `X` is the cardinality of one of its bases, or zero if its bases are 
  infinite -/
def r {α : Type*} (M : matroid_in α) (X : set α) : ℕ :=
  ⨅ (I : {I | M.basis I (X ∩ M.E)}), ncard (I : set α)
...
lemma r_le_card [finite α] (M : matroid_in α) (X : set α) : M.r X ≤ X.ncard :=
...
lemma indep_iff_r_eq_card [finite α] : M.indep I ↔ M.r I = I.ncard :=
indep_iff_r_eq_card_of_finite (to_finite _)

lemma rk_le_card_ground (M : matroid_in α) [finite M.E] : M.rk ≤ M.E.ncard :=
r_le_card_of_finite _ (to_finite _)
...
lemma r_fin.insert (hX : M.r_fin X) (e : α) : M.r_fin (insert e X) := (M.r_fin_singleton e).union hX 
...
lemma r_le_rk (M : matroid_in α) [finite_rk M] (X : set α) : M.r X ≤ M.rk := M.r_mono (subset_univ _)
...
lemma eq_of_r_eq_r_forall {M₁ M₂ : matroid_in α} [finite_rk M₁] (h : ∀ X, M₁.r X = M₂.r X) : M₁ = M₂ :=
begin
...
lemma r_union_le_add_r (M : matroid_in α) [finite_rk M] (X Y : set α) : M.r (X ∪ Y) ≤ M.r X + M.r Y :=
by linarith [M.r_submod X Y]

lemma r_union_le_card_add_r [finite E] (M : matroid_in α) (X Y : set α) : 
  M.r (X ∪ Y) ≤ X.ncard + M.r Y :=
(M.r_union_le_add_r X Y).trans (add_le_add_right (M.r_le_card _) _)

lemma r_union_le_r_add_card [finite E] (M : matroid_in α) (X Y : set α) : 
  M.r (X ∪ Y) ≤ M.r X + Y.ncard :=
(M.r_union_le_add_r X Y).trans (add_le_add_left (M.r_le_card _) _)

lemma rk_le_card_add_r_compl [finite E] (M : matroid_in α) (X : set α) : M.rk ≤ X.ncard + M.r Xᶜ :=
begin
...
lemma r_le_r_add_card_diff_of_subset [finite E] (M : matroid_in α) (hXY : X ⊆ Y) : 
  M.r Y ≤ M.r X + (Y \ X).ncard :=
(M.r_le_r_add_r_diff X Y).trans (add_le_add_left (by convert M.r_le_card (Y \ X)) _)

lemma r_add_card_le_r_add_card_of_subset [finite E] (M : matroid_in α) (hXY : X ⊆ Y) :
...
lemma exists_circuit_iff_card_lt_rk [finite E] : M.rk < (univ : set α).ncard ↔ ∃ C, M.circuit C :=
