...
begin
  ...
  { have h50 := @span_mono (zmod 2) _ _ _ _ _ _ (subset_univ (φ' '' (unif 2 4).E)),
    ...
    rw [finrank_finsupp, fintype.card_of_finset, finset.filter_congr_decidable, 
        filter_mem_univ_eq_to_finset, to_finset_card B, ← nat.card_eq_fintype_card, 
        ← ncard_def, (unif_base_iff (bit0_le_bit0.2 (nat.one_le_bit0_iff.2 
        (nat.lt_one_iff.2 rfl)))).1 hB] at h10, 
    ...
    have h12 := fintype.card_le_of_embedding (embedding_of_subset _ _ (subset_trans 
      ((φ'.subset_nonzero_of_simple U24_simple)) (@diff_subset_diff_left _ _ _ 
