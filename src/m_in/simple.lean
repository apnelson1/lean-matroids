...
variables {E E' : Type*} {N M : matroid_in E} {e f g : E} {X Y Z S T : set E}
...
def simple_on (M : matroid_in E) (X : set E) : Prop := ∀ ⦃e⦄, e ∈ X → ∀ ⦃f⦄, f ∈ X → M.indep {e, f}

/-- the property of a matroid_in having no loops or para pairs -/
def simple (M : matroid_in E) : Prop := ∀ e f, M.indep {e, f}
...
def para (M : matroid_in E) (e f : E) : Prop := M.nonloop e ∧ M.cl {e} = M.cl {f}
...
instance (M : matroid_in E) : is_per E M.para := 
{ symm  := λ _ _, para.symm,
  trans :=  λ _ _ _, para.trans }

/-- The set of elements parallel to `e` -/
def pcl (M : matroid_in E) (e : E) := {f | M.para e f}
...
lemma pcl_eq_cl_diff_loops (M : matroid_in E) (e : E) : M.pcl e = M.cl {e} \ M.cl ∅ :=
begin
...
lemma pcl_pairwise_disjoint (M : matroid_in E) : pairwise_disjoint (range M.pcl) id := 
begin
...
lemma sum_ncard_point_diff_loops [finite E] (M : matroid_in E) : 
...
def num_points (M : matroid_in E) := nat.card {P // M.point P}
